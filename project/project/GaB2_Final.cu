...
__global__ void initVNU(int *VtoC, int *receivedWord) {
...
// Implement variable-to-check message update
__global__ void VNU(int *VtoC, int *CtoV, int *receivedWord) {
    // Determine global thread index
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    // Check if thread index is within the range of VarNum
    if (idx < VarNum) {
...
__global__ void Checkdecide(int *decide, int *CtoV, int *receivedWord) {
	// Determine global thread index
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
    // Check if thread index is within the range of VarNum
	//	if (idx == 0)
...
__global__ void ComputeSyndrome1(int *decide, int* syndrome) {
	// Determine global thread index
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
		...
		for (nb = 0, nbtestedframes = 0; nb < NbMonteCarlo; nb++) {
			...
			for (iter = 0; iter < NbIter; iter++) {
				...
				if (iter == 0) {
					...
					// start keeping track of execution time
					//cudaEventRecord(startEvent, 0);
					initVNU<<<VarGrid,VarBlock>>>(deviceVtoC, deviceReceivedWord);
					...
					// start keeping track of execution time
					//cudaEventRecord(startEvent, 0);
					VNU<<<VarGrid,VarBlock>>>(deviceVtoC, deviceCtoV, deviceReceivedWord);
					CUDA_CHECK(cudaGetLastError());
					CUDA_CHECK(cudaDeviceSynchronize());
					// stop keeping track of execution time
					//cudaEventRecord(stopEvent, 0);
				...
				// start keeping track of execution time
				//cudaEventRecord(startEvent, 0);
				Checkdecide<<<VarGrid,VarBlock>>>(deviceDecide, deviceCtoV, deviceReceivedWord);
				CUDA_CHECK(cudaGetLastError());
				CUDA_CHECK(cudaDeviceSynchronize());
				// stop keeping track of execution time
				//cudaEventRecord(stopEvent, 0);
				...
				// start keeping track of execution time
				//cudaEventRecord(startEvent, 0);
				ComputeSyndrome1<<<GridNum,BlockNum>>>(deviceDecide, deviceSyndrome);
				CUDA_CHECK(cudaGetLastError());
				CUDA_CHECK(cudaDeviceSynchronize());
