...
__global__ void initVNU(int *VtoC, int *receivedWord, int size) {
...
// Implement variable-to-check message update
__global__ void VNU(int *VtoC, int *CtoV, int *receivedWord, int size) {
    // Determine global thread index
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int offset = columnDegree*idx;
    int i;
...
__global__ void Checkdecide(int *decide, int *CtoV, int *receivedWord, int size) {
	// Determine global thread index
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int offset = columnDegree*idx;
    int i;
    ...
    if (idx < size) {
        ...
        for (i = 0; i < columnDegree; i++) 
            shared_CtoV[offset+i] = (-2) * CtoV[interleaver[offset + i]] + 1;
        //    shared_CtoV[idx+columnDegree*i] = (-2) * CtoV[interleaver[offset + i]] + 1;
        //  __syncthreads();
        int Global = (1 - 2 * writeData);

        for (int i = 0; i < columnDegree; i++)
            Global += shared_CtoV[offset+i];
        //    Global += shared_CtoV[idx+columnDegree*i];
...
__global__ void ComputeSyndrome1(int *decide, int* syndrome, int size) {
	...
	if (idx < size) {
        ...
        //    shared_Decide[idx+rowDegree*i] = decide[columnIndex[offset+i]];
        __syncthreads();
		...
		//syndrome[idx] = shared_Decide[idx]^shared_Decide[idx+rowDegree*1]^
        //                shared_Decide[idx+rowDegree*2]^shared_Decide[idx+rowDegree*3]^
        //                shared_Decide[idx+rowDegree*4]^shared_Decide[idx+rowDegree*5]^
        //                shared_Decide[idx+rowDegree*6]^shared_Decide[idx+rowDegree*7];
    }
}
...
//	float alpha_max = 0.0200;		    // Channel Crossover Probability Max and Min
	...
	int *hostCtoV;
	int *hostDecide;
	int *hostReceivedWord;
	int *hostVtoC;
	int *hostCodeWord;
	int *hostSyndrome;
	int *hostSyndromeIntermediate;
	int *hostIsCodeword;
    // Allocate page-locked host memory for streaming
    //CUDA_CHECK(cudaMallocHost((void**)&hostReceivedWord, NUM_STRS*stream_size*sizeof(unsigned int)));
    CUDA_CHECK(cudaHostAlloc((void**)&hostVtoC, streamNum*streamSizeVar*columnDegree*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostCodeWord, streamNum*streamSizeVar*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostReceivedWord, streamNum*streamSizeVar*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostDecide, streamNum*streamSizeVar*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostCtoV, streamNum*streamSizeCheck*rowDegree*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostSyndrome, streamNum*streamSizeCheck*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostSyndromeIntermediate, GridNum*sizeof(unsigned int), cudaHostAllocDefault));
    CUDA_CHECK(cudaHostAlloc((void**)&hostIsCodeword, streamNum*sizeof(unsigned int), cudaHostAllocDefault));
	...
	for(alphaNum = 0, alpha = alpha_max; alpha >= alpha_min; alpha -= alpha_step, alphaNum++) {
		...
		for (nb = 0, nbtestedframes = 0; nb < NbMonteCarlo; nb++) {
			...
			//============================================================================
				// copy data to device
			//============================================================================
			//// Transfer data from host to device asynchronously for streamsNum;
            for (int j = 0; j < streamNum; j++) {
                CUDA_CHECK(cudaMemcpyAsync(deviceReceivedWord+(j*streamSizeVar), hostReceivedWord+(j*streamSizeVar), streamSizeVar*sizeof(unsigned int), cudaMemcpyHostToDevice, streams[j]));
                CUDA_CHECK(cudaMemcpyAsync(deviceCtoV+(j*streamSizeCheck*rowDegree), hostCtoV+(j*streamSizeCheck*rowDegree), streamSizeCheck*rowDegree*sizeof(unsigned int), cudaMemcpyHostToDevice, streams[j]));
                CUDA_CHECK(cudaMemcpyAsync(deviceDecide+(j*streamSizeVar), hostDecide+(j*streamSizeVar), streamSizeVar*sizeof(unsigned int), cudaMemcpyHostToDevice, streams[j]));
			...
			//	for (int j = 0; j < streamNum; j++) 
                if (iter == 0) {
                    ...
                    //cudaEventRecord(startEvent, 0);
                    for (int j = 0; j < streamNum; j++)
                        initVNU<<<VarGrid,VarBlock, 0, streams[j]>>>(deviceVtoC+(j*streamSizeVar*columnDegree), deviceReceivedWord+(j*streamSizeVar*columnDegree), streamSizeVar);
                    // stop keeping track of execution time
                    //cudaEventRecord(stopEvent, 0);
                    //cudaEventSynchronize(stopEvent);
                    //cudaEventElapsedTime(&elapsedTime, startEvent, stopEvent);
                    ...
                    //cudaEventRecord(startEvent, 0);
                    for (int j = 0; j < streamNum; j++)
                        VNU<<<VarGrid,VarBlock, streamSizeCheck*rowDegree*sizeof(int), streams[j]>>>(deviceVtoC+(j*streamSizeVar*columnDegree), deviceCtoV+(j*streamSizeCheck*rowDegree), deviceReceivedWord+(j*streamSizeVar), streamSizeVar);
                    // stop keeping track of execution time
                    //cudaEventRecord(stopEvent, 0);
                    //cudaEventSynchronize(stopEvent);
                    //cudaEventElapsedTime(&elapsedTime, startEvent, stopEvent);
                ...
                //cudaEventRecord(startEvent, 0);
                for (int j = 0; j < streamNum; j++)
                    CNU<<<CheckGrid,CheckBlock, streamSizeCheck*rowDegree*sizeof(int), streams[j]>>>(deviceCtoV+(j*streamSizeCheck*rowDegree), deviceVtoC+(j*streamSizeVar*columnDegree), streamSizeCheck);
                // stop keeping track of execution time
                //cudaEventRecord(stopEvent, 0);
                //cudaEventSynchronize(stopEvent);
                //cudaEventElapsedTime(&elapsedTime, startEvent, stopEvent);
                ...
                //cudaEventRecord(startEvent, 0);
                for (int j = 0; j < streamNum; j++)
                    Checkdecide<<<VarGrid,VarBlock, streamSizeVar*columnDegree*sizeof(int), streams[j]>>>(deviceDecide+(j*streamSizeVar), deviceCtoV+(j*streamSizeCheck*rowDegree), deviceReceivedWord+(j*streamSizeVar), streamSizeVar);
                // stop keeping track of execution time
                //cudaEventRecord(stopEvent, 0);
                //cudaEventSynchronize(stopEvent);
                //cudaEventElapsedTime(&elapsedTime, startEvent, stopEvent);
                ...
                //cudaEventRecord(startEvent, 0);
                for (int j = 0; j < streamNum; j++)
                    ComputeSyndrome1<<<GridNum,BlockNum, streamSizeCheck*rowDegree*sizeof(int), streams[j]>>>(deviceDecide+(j*streamSizeVar), deviceSyndrome+(j*streamSizeCheck), streamSizeCheck);
                //CUDA_CHECK(cudaGetLastError());
                //CUDA_CHECK(cudaDeviceSynchronize());
                //CUDA_CHECK(cudaMemcpy(hostSyndrome, deviceSyndrome, CheckNum*sizeof(int), cudaMemcpyDeviceToHost));
                for (int j = 0; j < streamNum; j++)
                    ComputeSyndrome2<<<GridNum/2,BlockNum,BlockNum*sizeof(int), streams[j]>>>(deviceSyndromeIntermediate+(j*GridNum/streamNum), deviceSyndrome+(j*streamSizeCheck));
                for (int j = 0; j < streamNum; j++)
                    ComputeSyndrome2<<<1,GridNum,GridNum*sizeof(int), streams[j]>>>(deviceNotCodeword, deviceSyndromeIntermediate+(j*GridNum/streamNum));
                //CUDA_CHECK(cudaGetLastError());
                //CUDA_CHECK(cudaDeviceSynchronize());
                // stop keeping track of execution time
                //cudaEventRecord(stopEvent, 0);
				...
				// copy data to host
                for (int j = 0; j < streamNum; j++) {
				    CUDA_CHECK(cudaMemcpyAsync(hostDecide+(j*streamSizeVar), deviceDecide+(j*streamSizeVar), streamSizeVar*sizeof(int), cudaMemcpyDeviceToHost, streams[j]));
                    CUDA_CHECK(cudaGetLastError());
                }
	...
	//CUDA_CHECK(cudaFreeHost(hostVtoC));
	CUDA_CHECK(cudaFreeHost(hostCtoV));
	CUDA_CHECK(cudaFreeHost(hostReceivedWord));
	CUDA_CHECK(cudaFreeHost(hostCodeWord));
	CUDA_CHECK(cudaFreeHost(hostDecide));
	CUDA_CHECK(cudaFreeHost(hostSyndrome));
	CUDA_CHECK(cudaFreeHost(hostSyndromeIntermediate));
	CUDA_CHECK(cudaFreeHost(hostIsCodeword));
	CUDA_CHECK(cudaGetLastError());
